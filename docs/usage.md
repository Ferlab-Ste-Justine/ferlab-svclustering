...
```bash
nextflow run ferlab/svclustering --input ./samplesheet.csv --outdir ./results --genome GRCh37 -profile docker
```
...
```bash
nextflow run ferlab/svclustering -profile docker -params-file params.yaml
```
...
```bash
nextflow pull ferlab/svclustering
```
